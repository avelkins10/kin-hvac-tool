...
interface Disclosure {
  id?: string
  name: string
  type: 'creditApplication' | 'prequalification' | 'txtMessageNotifications' | 'termsAndConditions'
  version: string
  summary: string
  text: string
...
export function DisclosureCheckboxes({
  ...
  useEffect(() => {
    async function fetchDisclosures() {
      try {
        const res = await fetch(`/api/finance/lightreach/disclosures?language=${language}`)
        if (!res.ok) {
  ...
  const sanitizeHTML = (html: string) => {
    return DOMPurify.sanitize(html, {
      ALLOWED_TAGS: ['p', 'br', 'strong', 'em', 'b', 'i', 'u', 'ul', 'ol', 'li', 'a', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'span', 'div'],
  ...
  return (
    <div className="space-y-4" role="group" aria-labelledby="disclosures-heading">
      <div className="space-y-1">
        <Label id="disclosures-heading" className="text-base font-medium">Required Disclosures</Label>
        <p className="text-sm text-muted-foreground" id="disclosures-description">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <CollapsibleTrigger asChild>
                      <Button
                        ...
                        aria-label={expanded[disclosure.type] ? `Collapse ${disclosure.name} details` : `Expand ${disclosure.name} details`}
                      >
              ...
              <CollapsibleContent>
                <div className="mt-3 ml-7 p-3 rounded bg-muted/50 text-sm">
                  <div
                    ...
                    dangerouslySetInnerHTML={{ __html: sanitizeHTML(disclosure.text) }}
                  />
 ...
 */
export function useDisclosuresValid(acceptedDisclosures: AcceptedDisclosure[]): boolean {
