...
import { lightReachClient, HVACSystemDesign } from '@/lib/integrations/lightreach'
import {
  FinanceError,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (typeof totalFinancedAmount !== 'number' || totalFinancedAmount <= 0) {
      return NextResponse.json(
        { error: 'totalFinancedAmount is required and must be a positive number' },
        { status: 400 }
      )
    }
...
function generateMockPricing(totalFinancedAmount: number) {
  ...
  return plans.map((plan, index) => {
    ...
    let currentPayment = Math.round(totalFinancedAmount * plan.factor * 100) / 100
    let totalPaid = 0
