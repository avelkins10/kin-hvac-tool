...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      ...
      proposal.companyId !== session.user.companyId
    ) {
    ...
    if (submittingUser?.lightreachSalesRepName?.trim()) {
      applicationData.salesRepName =
        submittingUser.lightreachSalesRepName.trim();
    }
    if (submittingUser?.lightreachSalesRepEmail?.trim()) {
      applicationData.salesRepEmail =
        submittingUser.lightreachSalesRepEmail.trim();
    }
    if (submittingUser?.lightreachSalesRepPhone?.trim()) {
      applicationData.salesRepPhoneNumber =
        submittingUser.lightreachSalesRepPhone.trim();
    }
    ...
    if (existingApplication) {
      const daysSinceCreation = Math.floor(
        (Date.now() - existingApplication.createdAt.getTime()) /
          (1000 * 60 * 60 * 24),
      );
      ...
      if (daysSinceCreation < 7) {
        return NextResponse.json(
          {
            error:
              "An active finance application already exists for this proposal",
    ...
    if (enableTestMode) {
      ...
      const factorKey =
        `${termYears}yr_${escalatorRate}%` as keyof typeof paymentFactors;
      const paymentFactor =
        paymentFactors[factorKey] || paymentFactors["10yr_0%"];
      ...
      const monthlyPayment =
        Math.round(systemPrice * paymentFactor * 100) / 100;
